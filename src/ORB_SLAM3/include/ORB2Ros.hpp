...
class ORB2Ros : public rclcpp::Node
{
public:
  ORB2Ros();
  virtual ~ORB2Ros() = default;
...
private:
    ...
    int count_im_buffer = 0; // count dropped frames
  	...
  	// consts
  	const std::string pub_topic_name = "camera_pose";

  	// functions from orb_slam
...
ORB2Ros::ORB2Ros() : Node("orb2ros_node")
{
    ...
    this->get_parameter("Camera_yaml", camera_cfg);
    cout << "vocabulary: " << vocabulary << endl;
    cout << "camera_cfg: " << camera_cfg << endl;
    ORB_SLAM3::System SLAM(vocabulary, camera_cfg, ORB_SLAM3::System::IMU_STEREO, true, 0, file_name);
    float imageScale = SLAM.GetImageScale();
    std::cout << "Initialized SLAM" << std::endl;
    double timestamp;
    cv::Mat im, imRight;
...
rs2_option ORB2Ros::get_sensor_option(const rs2::sensor& sensor)
{
...
rs2_vector ORB2Ros::interpolateMeasure(const double target_time,
                              const rs2_vector current_data, const double current_time,
                              const rs2_vector prev_data, const double prev_time)
{
