...
class PoseToMarkerNode(Node):
    def __init__(self):
        ...
        self.subscription_wrist = self.create_subscription(
            Pose, '/wrist_pose', self.pose_wrist_callback, 10
        )
        # 发布 Marker 话题
        ...
        self.marker_wrist_pub = self.create_publisher(Marker, '/wrist_pose_marker', 10) 
    ...
    def pose_wrist_callback(self, msg):
        # 创建 Marker 消息
        marker = Marker()
        marker.header = Header(frame_id='map', stamp=self.get_clock().now().to_msg())
        marker.ns = 'pose_wrist_marker'
        marker.id = 0
        marker.type = Marker.ARROW  # 使用箭头表示 Pose
        marker.action = Marker.ADD
        marker.pose = msg  # 直接使用 Pose 消息
        ...
        # 发布 Marker
        self.marker_wrist_pub.publish(marker)
